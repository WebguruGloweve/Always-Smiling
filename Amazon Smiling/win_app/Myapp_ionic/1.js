 ...
 * Search geometries.
 *
 * @param geoRange coordinate range to search
 */
var getMapboxFromWKT = function (wkt, options) {
        var parts = wkt.split('(');
        var last = parts.pop();
        parts = last.split(")");
        var first = parts[0];

        var coordinates = first.split(',');
...
var searchGeometries = function(drawItems, wkt, options){
    drawItems.clearLayers();
    var url = basePath + baseUrl + '/cartography/search';
    // var url = basePath + baseUrl + '/cartography/81/geometries';
    var _drawItems = drawItems;
    var data = {
        mediaId: currentMediaId(),
        geo: {mapbox: getMapboxFromWKT(wkt, options)},
        options: options
    };
    $.get(url, data)
        .done(function(data) {
            ...
            if (data.geometries.length) {
                displayGeometries(data.geometries, _drawItems);
		annotateGeometries(_drawItems._map, 'geobrowse', _drawItems);
            }

            // Useful for data processing just after the drawnItems are added to
            // the map.
            // if (drawnItems && drawnItems._map && drawnItems._map.fireEvent) {
            //     drawnItems._map.fireEvent('searchGeometries:done', {
            //         returnData: data,
        ...
        .fail(function(jqxhr) {
            var message = (jqxhr.responseText && jqxhr.responseText.substring(0, 1) !== '<')
                ? JSON.parse(jqxhr.responseText).message
                : Omeka.jsTranslate('Unable to search the geometries.');
            alert(message);
        });
}
...
//=====================================================================================================
            var geojson = element.layer.toGeoJSON();
            var wkt;
            // Check the process of terraformer wkt convert / reconvert (Feature/FeatureCollection).
            if (geojson.features && geojson.features[0].geometry) {
                wkt = Terraformer.WKT.convert(geojson.features[0].geometry);
            ...
            var options = {};
            prepareSaveOptions(element.layer, options);
            searchGeometries(drawnItems, wkt,options);
        }
    });

    if (! userRights.create) return;
